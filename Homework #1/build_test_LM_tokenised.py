...
# tokenise by:
# 1. split by character, including special char, but excluding full stop
# 2. join back with spacing
def tokenise(line):
    stripped = nltk.tokenize.simple.CharTokenizer().tokenize(line)
    words = " ".join(stripped)
    return words

def get_four_gram(words):
    if len(words) == 4:
        token = words # "END" marked by fullstop.
        words = ""
    else:
        token = words[:4]
...
def build_LM(in_file):
    ...
    for line in lines:
        ...
        tokenised = tokenise(line[index_of_first_space:])
        words = tokenised[index_of_first_space:] # includes first spacing
        ...
        while (token):
            if token not in tokens: # first encounter of token
                tokens.append(token)
                token_langs[token] = {}
...
def test_LM(in_file, out_file, LM):
    ...
    for line in lines:
        ...
        words = " " + tokenised # simulate "START" token, like training set
        token, words = get_four_gram(words)
